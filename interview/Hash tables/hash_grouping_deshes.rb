# You are given a list dishes, 
# where each element consists of a list of strings beginning with the name of the dish, 
# followed by all the ingredients used in preparing it. 
...
def groupingDishes(dishes)
    ...
    dishes.each do |dish,*ingred|
        ingred.each { |ing| index[ing] += [dish] }
    end
    
    index.to_a.select { |i,ds| ds.size > 1 }
        .each { |i,ds| ds.sort! }
        .map(&:flatten).sort
end