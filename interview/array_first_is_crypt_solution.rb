...
# the goal is to find the correspondence between letters and digits, 
# such that the given arithmetic equation consisting of letters holds true 
...
# between letters and numbers in the cryptarithm. 
# The first character in the pair is an uppercase English letter, 
...
def isCryptSolution(crypt, solution)
    converts = crypt.map{|term| term.chars.map{|char| solution.to_h[char]}.join('')}
    (converts.select{|str| "#{str.to_i}".to_s == str } == converts and (converts[0].to_i + converts[1].to_i == converts[2].to_i))
end